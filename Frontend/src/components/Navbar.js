  ...
  useEffect(() => {
    const checkLoggedInStatus = async () => {
      try {
        ...
        if (token) {
          const responsePatient = await fetch('http://medimind.in.net:27017/api/v1/patient/validateTokenPatient', {
            method: 'POST',
            headers: {
          ...
          if (responsePatient.ok) {
            ...
            const responseConsultant = await fetch('http://medimind.in.net:27017/api/v1/consultant/validateTokenConsultant', {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${token}`
              }
